...
def build_model(lr, l2, activation='sigmoid'):
    ...
    # model = keras.utils.multi_gpu_model(Model([img_a, img_b], x), gpus=4)
    model = orig_model
    model.compile(optim, loss='binary_crossentropy', metrics=['binary_crossentropy', 'acc'])
    return model, branch_model, head_model, orig_model


model, branch_model, head_model, orig_model = build_model(64e-5, 0)
head_model.summary()
...
class FeatureGen(Sequence):
    def __init__(self, data, batch_size=512, verbose=1):
...
def compute_score(verbose=1):
    ...
    """
    features = branch_model.predict_generator(FeatureGen(train, verbose=verbose), max_queue_size=12, workers=24,
                                              verbose=0, use_multiprocessing=True)
    score = head_model.predict_generator(ScoreGen(features, verbose=verbose), max_queue_size=12, workers=24, use_multiprocessing=True, verbose=0)
...
def compute_acc(val, prediction, golden, top_k=5):
    ...
    print("\n")
    print("\n")
    print("50 ACC TOP %d:" % top_k, true_counts / float(total_counts))
    print("\n")
    print("\n")
    print("\n")
    print("\n")
...
if isfile(model_name + '-50.model'):
# if False:
    tmp = keras.models.load_model(model_name + '-50.model')
    ...
    score = head_model.predict_generator(ScoreGen(fknown, fval, batch_size=4096), max_queue_size=20, workers=10, verbose=0)
    score = score_reshape(score, fknown, fval)
    # print("Here")
    vtop, vhigh, pos, prediction = prepare_submission(0.9999, 'mpiotte-standard-50.csv')
